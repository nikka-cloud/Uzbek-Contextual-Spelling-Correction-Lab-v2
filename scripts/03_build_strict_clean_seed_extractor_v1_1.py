...
OUT_DIR = ROOT / "strict_clean_seed_extractor_v1_1"
...
KNOWN_BAD_TOKEN_RE = re.compile(
    ...
    r"qechki|xromosom|tomonidai|shirinmibilolmay|holatikasal|hayota|"
    r"quydida|birikki|sho\s+roi|sxolastiq|toychi|bonuga|turlituman|"
...
SUSPICIOUS_ENDING_RE = re.compile(
    r"(tirida|veradiqaynar|bilolmay|atrofimizga|yebichishni|g'erlardan)$",
    re.I,
)
...
def strict_reasons(text: str, p_usable: float, profile: dict) -> list[str]:
    ...
    if KNOWN_BAD_TOKEN_RE.search(text):
        reasons.append("known_bad_token_or_fragment")
...
def main() -> None:
    ...
    if not ROUTER_OOF.exists():
        raise FileNotFoundError(ROUTER_OOF)
    ...
    for profile in PROFILES:
        ...
        recall = selected_healthy / total_healthy if total_healthy else 0.0
    ...
    summary_df.to_csv(OUT_DIR / "profile_comparison_v1_1.csv", index=False)

    lines = []
    lines.append("# Strict Clean Seed Extractor v1.1 Profile Comparison")
    lines.append("")
    ...
    best = sorted(
        all_summaries,
        key=lambda r: (r["precision"], r["selected_candidates"]),
        reverse=True,
    )[0]
    ...
    if best["precision"] >= 0.95 and best["selected_candidates"] >= 30:
        lines.append(
            f"Best profile `{best['profile']}` reaches >=95% calibration precision with enough candidates. "
            "Next step: manually review its sample. Do not touch validation yet."
        )
    elif best["precision"] >= 0.95:
        lines.append(
        ...
        lines.append(
            "No profile reaches 95% calibration precision. We still need stronger lexical or language-quality filtering."
        )

    summary_text = "\n".join(lines)
    (OUT_DIR / "profile_comparison_v1_1.md").write_text(summary_text, encoding="utf-8")
